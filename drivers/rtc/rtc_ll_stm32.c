...
// #define STM32_RTC_CLK_SRC_LSI
#define STM32_RTC_CLK_SRC_LSE
...
#define DIV_ROUND_CLOSEST(numerator, denominator) \
...
static int rtc_stm32_init(const struct device *dev)
{
	...
	if (!device_is_ready(clk)) {
		printf("clock control device not ready");
		return -ENODEV;
	}

	/* Enable RTC bus clock */
	if (clock_control_on(clk, (clock_control_subsys_t)&cfg->pclken) != 0) {
		printf("clock op failed\n");
		return -EIO;
	}
