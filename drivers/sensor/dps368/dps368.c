/* Driver for Infineon DPS368 temperature and pressure sensor */
...
#define DT_DRV_COMPAT infineon_dps368
...
LOG_MODULE_REGISTER(DPS368, CONFIG_SENSOR_LOG_LEVEL);
 ...
 * structure
 */
static void dps368_calib_coeff_creation(const uint8_t raw_coef[18],
					struct dps368_cal_coeff *comp)
{
...
static bool poll_rdy(struct dps368_data *data, const struct dps368_cfg *config,
...
static bool dps368_trigger_temperature(struct dps368_data *data,
				       const struct dps368_cfg *config)
{
	...
	if (!poll_rdy(data, config, IFX_DPS368_REG_ADDR_MEAS_CFG,
...
static bool dps368_trigger_pressure(struct dps368_data *data,
				    const struct dps368_cfg *config)
{
 ...
 */
static void dps368_hw_bug_fix(struct dps368_data *data,
			      const struct dps368_cfg *config)
{
 ...
 */
static void dps368_scale_temperature(int32_t tmp_raw, struct dps368_data *data)
{
 ...
 */
static void dps368_scale_pressure(int32_t tmp_raw, int32_t psr_raw,
				  struct dps368_data *data)
{
	const struct dps368_cal_coeff *comp = &data->comp;
...
static bool dps368_measure_tmp_psr(struct dps368_data *data,
				   const struct dps368_cfg *config)
{
	if (!dps368_trigger_temperature(data, config)) {
		return false;
	}

	if (!dps368_trigger_pressure(data, config)) {
		return false;
	}
	...
	dps368_scale_temperature(data->raw_tmp, data);
	dps368_scale_pressure(data->raw_tmp, psr_raw, data);
 ...
 */
static bool dps368_measure_psr(struct dps368_data *data,
			       const struct dps368_cfg *config)
{
	/* measure pressure */
	if (!dps368_trigger_pressure(data, config)) {
...
static bool dps368_measure_tmp(struct dps368_data *data,
			       const struct dps368_cfg *config)
{
	/* measure temperature */
	if (!dps368_trigger_temperature(data, config)) {
...
static int dps368_init(const struct device *dev)
{
	struct dps368_data *data = dev->data;
	const struct dps368_cfg *config = dev->config;
	...
	dps368_hw_bug_fix(data, config);
	dps368_measure_tmp_psr(data, config);
...
static int dps368_sample_fetch(const struct device *dev,
			       enum sensor_channel chan)
{
	struct dps368_data *data = dev->data;
	const struct dps368_cfg *config = dev->config;

	LOG_DBG("Fetching sample from DPS368");

	switch (chan) {
	case SENSOR_CHAN_AMBIENT_TEMP:
		if (!dps368_measure_tmp(data, config)) {
			LOG_ERR("Failed to measure temperature");
			return -EIO;
		}
		break;
	case SENSOR_CHAN_PRESS:
		if (!dps368_measure_psr(data, config)) {
			LOG_ERR("Failed to measure pressure");
			return -EIO;
		}
		break;
	case SENSOR_CHAN_ALL:
		if (!dps368_measure_tmp_psr(data, config)) {
...
static int dps368_channel_get(const struct device *dev,
			      enum sensor_channel chan,
...
static const struct sensor_driver_api dps368_api_funcs = {
	.sample_fetch = dps368_sample_fetch,
	.channel_get = dps368_channel_get,
};

/* support up to 2 instances */

#if DT_NODE_HAS_STATUS(DT_DRV_INST(0), okay)
static struct dps368_data dps368_data_0;
static const struct dps368_cfg dps368_cfg_0 = {
	.i2c_bus_name = DT_INST_BUS_LABEL(0),
	.i2c_addr = DT_INST_REG_ADDR(0)
};

DEVICE_DT_INST_DEFINE(0, dps368_init, NULL,
		    &dps368_data_0, &dps368_cfg_0, POST_KERNEL,
		    CONFIG_SENSOR_INIT_PRIORITY, &dps368_api_funcs);
#endif

#if DT_NODE_HAS_STATUS(DT_DRV_INST(1), okay)
static struct dps368_data dps368_data_1;
static const struct dps368_cfg dps368_cfg_1 = {
	.i2c_bus_name = DT_INST_BUS_LABEL(1),
	.i2c_addr = DT_INST_REG_ADDR(1)
};

DEVICE_DT_INST_DEFINE(1, dps368_init, NULL,
		    &dps368_data_1, &dps368_cfg_1, POST_KERNEL,
		    CONFIG_SENSOR_INIT_PRIORITY, &dps368_api_funcs);
#endif
