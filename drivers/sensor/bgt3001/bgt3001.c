...
static void bgt3001_uart_isr(const struct device *uart_dev, void *user_data)
{
	...
	if (uart_irq_rx_ready(uart_dev)) {
		...
		uart_irq_rx_disable(uart_dev);
			k_sem_give(&bgt3001.rx_sem);
			if (bgt3001.has_rsp) {
				k_sem_give(&bgt3001.tx_sem);
			}
		}
		j++;
	}
			     ...
			     const bool has_rsp)
{
	...
	ret = k_sem_take(&d->tx_sem, BGT3001_WAIT);
	if (ret) {
		return ret;
	}
	...
	if (has_rsp) {
		...
		ret = k_sem_take(&d->rx_sem, BGT3001_WAIT);
#ifdef CONFIG_BGT3001_KEEP_MCU_AWAKE
...
{	uint16_t dt;
	uint8_t d[2];
	d[1] = bgt3001.rd_data[cmd][i];
	d[0] =bgt3001.rd_data[cmd][i - 1];
	dt =sys_get_be16(&d);
